...
#include UE_INLINE_GENERATED_CPP_BY_NAME(EotRDamageExecution)
...
struct FDamageStatics
{
	...
	FDamageStatics()
	{
		BaseDamageDef = FGameplayEffectAttributeCaptureDefinition(UEotRCombatSet::GetBaseDamageAttribute(), EGameplayEffectAttributeCaptureSource::Source, true);
	}
};
...
UEotRDamageExecution::UEotRDamageExecution()
{
	RelevantAttributesToCapture.Add(DamageStatics().BaseDamageDef);
}

void UEotRDamageExecution::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
#if WITH_SERVER_CODE
	const FGameplayEffectSpec& Spec = ExecutionParams.GetOwningSpec();
	FEotRGameplayEffectContext* TypedContext = FEotRGameplayEffectContext::ExtractEffectContext(Spec.GetContext());
	check(TypedContext);
	...
	{
		/*
		UEotRTeamSubsystem* TeamSubsystem = HitActor->GetWorld()->GetSubsystem<UEotRTeamSubsystem>();
	...
	{
		UE_LOG(LogEotRAbilitySystem, Error, TEXT("Damage Calculation cannot deduce a source location for damage coming from %s; Falling back to WORLD_MAX dist!"), *GetPathNameSafe(Spec.Def));
	}

	// Apply ability source modifiers
	float PhysicalMaterialAttenuation = 1.0f;
	float DistanceAttenuation = 1.0f;
	if (const IEotRAbilitySourceInterface* AbilitySource = TypedContext->GetAbilitySource())
	{
	...
	if (DamageDone > 0.0f)
	{
		...
		OutExecutionOutput.AddOutputModifier(FGameplayModifierEvaluatedData(UEotRHealthSet::GetDamageAttribute(), EGameplayModOp::Additive, DamageDone));
	}
