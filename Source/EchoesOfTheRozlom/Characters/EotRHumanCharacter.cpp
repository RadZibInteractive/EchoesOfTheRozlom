...
void AEotRHumanCharacter::PostInitializeComponents()
{
	...
	FirstPersonCamera->AttachToComponent(
		...
		HumanData->FirstPersonCameraSocketName
	);
...
void AEotRHumanCharacter::PossessedBy(AController* NewController)
{
...
void AEotRHumanCharacter::UnPossessed()
{
	Super::UnPossessed();
...
float AEotRHumanCharacter::CalculateCameraInput(bool bIsYawAxis, float ActionValue) const
{
	...
	if (bIsYawAxis)
	{
		const float MeshYaw = GetMesh()->GetSocketRotation(TEXT("root")).Yaw;
		const float TargetYaw = MeshYaw + 90.f;

		const float ControlYaw = GetControlRotation().Yaw;

		Delta = UKismetMathLibrary::NormalizeAxis(ControlYaw) - UKismetMathLibrary::NormalizeAxis(TargetYaw);
	}
	else
	{
